...
const dataset = JSON.parse(JSON.stringify(require('../TestData/endtoend.json')))


test(`Create an order ${dataset.productName}`, async ({ page }) => {
    ...
    await signInPage.launchUrl(dataset.url)
    await signInPage.validLogin(dataset.username, dataset.password)
