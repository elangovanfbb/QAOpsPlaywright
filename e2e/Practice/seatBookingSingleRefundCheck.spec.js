...
test("SEAT BOOKING", async({page})=>
{
  ...
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}T${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
    ...
    // Click on View Details and navigate to View Details page

    const myOwnEventName = page.locator('#booking-card').filter({ hasText: bookingRefNum });
    await myOwnEventName.getByRole("button", {name : 'View Details'} ).click()
  // await myOwnEvent.locator('button').filter({ hasText: 'View Details' }).click()
  await page.getByRole("button", {name : 'Check eligibility for refund?'}).click()

  expect(await page.locator("[aria-label='Loading']").isVisible()).toBeTruthy()

  const refundResult = await page.locator("#refund-result")
  await refundResult.waitFor()
  expect(await refundResult.isVisible()).toBeTruthy()
  console.log(await refundResult.textContent())

  await expect(await refundResult.textContent()).toContain('Eligible for refund');
await expect(await refundResult.textContent()).toContain('Single-ticket bookings qualify for a full refund');
